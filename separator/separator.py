...
class Separator:
    ...
    def __init__(
        ...
        log_formatter=None,
        model_file_dir="/tmp/VoiceChange-models/",
        output_dir=None,
        output_format="WAV",
        output_bitrate=None,
        ...
        if not info_only:
            package_version = self.get_package_distribution("VoiceChange").version
        ...
        env_model_dir = os.environ.get("VOICECHANGE_MODEL_DIR")
        if env_model_dir:
            self.model_file_dir = env_model_dir
            self.logger.info(f"Using model directory from VOICECHANGE_MODEL_DIR env var: {self.model_file_dir}")
    ...
    def list_supported_model_files(self):
        """
        This method lists the supported model files for VoiceChange, by fetching the same file UVR uses to list these.
        ...
        model_scores = {}
        try:
            with resources.open_text("VoiceChange", "models-scores.json") as f:
        ...
        self.logger.debug(f"VoiceChange model list loaded")
